...
auto CalculateColumn(const AbstractExpressionRef &function, WindowFunctionType type,
                     const std::vector<std::pair<OrderByType, AbstractExpressionRef>> &order_by, TypeId column_type,
                     bool repeat_value, const Schema &schema, std::map<uint32_t, std::vector<Tuple>> &partitions,
      ...
      if (!tmp.IsNull()) {
        switch (type) {
          ...
          case WindowFunctionType::Rank:
            ...
            auto it = std::find_if(part.second.cbegin(), part.second.cend(), [&](const Tuple &t) {
    ...
    if (column_value_expression->GetColIdx() < child_executor_->GetOutputSchema().GetColumns().size()) {
      for (const auto &tuple : partitions[idx][0]) {
        column_values.push_back(
            tuple.GetValue(&child_executor_->GetOutputSchema(), column_value_expression->GetColIdx()));
      }
    } else {
